...
## to fix compiltation bug for CLHEP with latest GCC version missing cstdint include file

add_include_cstdint_statement() {
    local file_path=$(find . -type f -name "RandomEngine.h")
 
    if [ -n "$file_path" ]; then
        for file in $file_path; do
            if ! grep -qE '^\s*#\s*include\s*<cstdint>' "$file"; then
                sed -i '/#ifndef HepRandomEngine_h/{:loop;N;/#define HepRandomEngine_h 1/!b loop;s/#define HepRandomEngine_h 1/#define HepRandomEngine_h 1\n#include <cstdint>/}' "$file"
                echo "Include statement added to $file"
            else
                echo "Include statement already exists in $file"
            fi
        done
    else
        echo "File RandomEngine.h not found."
    fi
}
...
$CMake_path \
-DCFLAGS_C=-Wmissing-prototypes \
-DCFLAGS_CXX=-std=c++0x \
-DLDFLAGS= \
-DLIBS= \
